...
const schema = z.object({
  name: z
    ...
    .regex(k8sNameRegex, 'Name must consist of lowercase alphanumeric characters or hyphens, and start/end with an alphanumeric'),
  ...
  const handleYamlChange = (value: string) => {
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object') {
        ...
        if (spec) {
          const containers = (
            (spec.configurationSpec as Record<string, unknown>)?.template as Record<string, unknown>
          ...
          const volumes = spec.volumesSpec as Record<string, unknown> | undefined;
          ...
          reset({
            ...
            envVars: ((container.env as Array<{ name: string; value: string }>) ?? []).map(
              (e) => ({ key: e.name, value: e.value })
            ),
            ...
            nfsVolumes: ((volumes?.nfsVolumes as Array<Record<string, unknown>>) ?? []).map(
              (v) => {
                const storage = ((v.capacity as Record<string, string>)?.storage ?? '1Gi');
    ...
    <form onSubmit={handleSubmit(handleFormSubmit)} className="flex flex-col gap-4">
      ...
      {activeTab === 'form' ? (
        <div className="flex flex-col gap-3">
          ...
          <Accordion type="multiple" defaultValue={['details', 'configuration']}>
