...
import { CappFormValues, NFSVolumeFormValue, SecretVolumeFormValue, ConfigMapVolumeFormValue, VolumeMountFormValue } from '../CappForm';
...
const emptyVolumeMount = (): VolumeMountFormValue => ({ volumeName: '', mountPath: '' });
...
export const VolumesSection: React.FC<VolumesSectionProps> = ({ control, watch, setValue, namespace }) => {
  ...
  const volumeMounts = watch('volumeMounts') as VolumeMountFormValue[];
  ...
  const addVolumeMount = () => setValue('volumeMounts', [...volumeMounts, emptyVolumeMount()]);
  const removeVolumeMount = (i: number) => setValue('volumeMounts', volumeMounts.filter((_, idx) => idx !== i));
  ...
  return (
    <SectionAccordion value="volumes" title="Volumes">
      <div className="flex flex-col gap-6">
        ...
        {/* Volume Mounts (required to mount NFS volumes) */}
        <div className="flex flex-col gap-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <Controller name={`volumeMounts.${index}.volumeName` as keyof CappFormValues} control={control} render={({ field }) => (
                  <div className="flex flex-col gap-1">
                    ...
                    <Select value={field.value as string} onValueChange={field.onChange}>
                      <SelectTrigger className="bg-card border-border"><SelectValue placeholder="Select NFS volume" /></SelectTrigger>
                      <SelectContent>
                        {nfsVolumes.filter((v) => v.name).map((v) => <SelectItem key={v.name} value={v.name}>{v.name}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
                )} />
                <Controller name={`volumeMounts.${index}.mountPath` as keyof CappFormValues} control={control} render={({ field }) => (
                  <Input label="Mount Path" placeholder="/data" value={field.value as string} onChange={field.onChange} />
                )} />
              </div>
            </div>
          ))}
          <Button type="button" variant="secondary" size="sm" onClick={addVolumeMount} icon={<Plus size={14} />}>Add Volume Mount</Button>
        </div>
